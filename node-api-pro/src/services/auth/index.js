...
export const githubLoginRouteHandler = async (req, res) => {
    const {
        PATHFIX_URL,
        PATHFIX_PUBLIC_KEY,
        PATHFIX_PRIVATE_KEY,
    ...
    const uri = `${PATHFIX_URL}?user_id=${userId}&public_key=${PATHFIX_PUBLIC_KEY}&private_key=${PATHFIX_PRIVATE_KEY}`
    ...
    console.log(resp)
    if (resp?.data?.pincStatus === "error") {
        return res.status(400).json({message: res?.data?.errorMessage});
    }
